...
#this is for the broad analysis
...
taxa_data$month <- lubridate::month(taxa_data$TO_STARTDATE)
head(taxa_data)
...
#moths is such a huge dataset, reduce the sample size
if(grepl("Moths", mytaxa)){
  selectVisits <- sample(unique(taxa_data$visit), size = 100000)
...
subsampleData <- function(visit_data){
  ...
  #get 10 km grids in which a species is present
  ...
  #get period of year when species is detectable
